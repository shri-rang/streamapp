...
class HomeScreenGenreMoviesList extends StatelessWidget {
  ...
  HomeScreenGenreMoviesList(
      {required this.genreMoviesList, required this.isDark});
  ...
  Widget build(BuildContext context) {
    ...
    return SliverList(
        delegate: SliverChildListDelegate(List.generate(
            genreMoviesList!.length,
            (index) => buildList(context, genreMoviesList![index],
                genreMoviesList![index].videos!)).toList()));
  }

  Widget buildList(BuildContext context,
      FeaturesGenreAndMovies featuresGenreAndMovies, List<Movie> moviesList) {
    return Padding(
                      ...
                      onTap: () {
                        switch (moviesList[index].isTvseries) {
                          ...
                          case "0":
                            {
                              Navigator.pushNamed(
                                  context, MovieDetailScreen.route, arguments: {
                                "movieID": moviesList[index].videosId
                              });
                            }
                            break;
